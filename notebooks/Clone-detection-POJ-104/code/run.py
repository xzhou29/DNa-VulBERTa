...
def main():
    ...
    if 'tokenizer.model' in args.tokenizer_name:
        # bert_dir = '/scratch/cbert/bert_source/'
        # model_path_ = '{}/pretrained-bert'.format(bert_dir)
        # tokenizer = BertTokenizerFast.from_pretrained( args.tokenizer_name)
        tokenizer = transformers.AlbertTokenizer.from_pretrained(args.tokenizer_name)
    ...
    if args.model_name_or_path:
        model = model_class.from_pretrained(args.model_name_or_path,
                                            ...
                                            cache_dir=args.cache_dir if args.cache_dir else None) 
    # TODO: HARD CODE 
    elif 'pretrained-dna-albnert' in args.model_name_or_path:
        # checkpoint = '{}/pretrained-bert'.format(bert_dir)
        # checkpoint = os.path.join(checkpoint, "checkpoint-60000")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        checkpoint_prefix = os.path.join('checkpoint-best-map', 'model.bin')
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        checkpoint_prefix = os.path.join('checkpoint-best-map', 'model.bin') #'checkpoint-best-map/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        model.load_state_dict(torch.load(output_dir),strict=False)                  
        model.to(args.device)
